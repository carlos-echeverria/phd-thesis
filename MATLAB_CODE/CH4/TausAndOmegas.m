function [ tauu, omegaa ] = TausAndOmegas( blkA )
%TAUSANDOMEGAS calculates the taus and omegas for all refinement steps.
%
% function call: 
%
...
%
%   tauu_{i,t} = ||blkA_{i,i}^{-1}*blkA_{i,i+1}||/(1-||blkA_{i,i}^{-1}*blkA_{i,i-1}||*tau_{i-1,t-1})
% omegaa_{i,t} = ||blkA_{i,i}^{-1}*blkA_{i,i-1}||/(1-||blkA_{i,i}^{-1}*blkA_{i,i+1}||*omega_{i-1,t-1})
%
...
%% build taus and omegas for Theorem 2.6
 ...
 for k = 2:M-1
     ...
     omega{k} = norm(inv(blkA{k,k})*blkA{k,k-1})/(1-(norm(inv(blkA{k,k})*blkA{k,k+1})));   
 end
  
     omega{M} = norm(inv(blkA{M,M})*blkA{M,M-1});
       tau{M} = 0; 
...
%% build iterative taus and omegas for Theorem 2.7
    ...
    omegaa = cell(M,M-1);
     ...
     for k=1:M-1
         ...
         omegaa{1,k} = 0; 
         omegaa{k,1} = omega{k};
         omegaa{M,k} = norm(inv(blkA{M,M})*blkA{M,M-1});
        ...
        for ii=2:M-1
            ...
            if ii>M-tt+1, 
                 omegaa{ii,tt} = omegaa{ii,tt-1};
                  ...
                  omegaa{ii,tt} = norm(inv(blkA{ii,ii})*blkA{ii,ii-1})/(1-(norm(inv(blkA{ii,ii})*blkA{ii,ii+1})*omegaa{ii+1,tt-1}));
            end
        end   
    end
